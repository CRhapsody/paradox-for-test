...
import torch
from provable_training.memory_calculate import pp_cuda_mem
"""
...
def backward_deeppoly(net, layer_idx, expr_coeff, layer_sizes, layer_bounds, inputs, eps, data_range, skip_ub=False, lambda_ls=None, crown_variant=None, soft_slope_gamma=None):
    ...
    if layer_idx != len(net.layers)-1:
        print(pp_cuda_mem(f'Before backsub {layer_idx}'))
    else:
        print(pp_cuda_mem(f'Before Final backsub'))
    l_coeff = expr_coeff
    ...
    for j in range(layer_idx, -1, -1):
        ...
        if isinstance(layer, nn.Linear):
            ...
            print(pp_cuda_mem(f'Before {layer_idx} relu {j}'))
            ...
            print(pp_cuda_mem(f'After {layer_idx} relu {j}'))
        elif isinstance(layer, nn.Conv2d):	
            print(pp_cuda_mem(f'Before {layer_idx} conv2d {j}'))
            ...
            print(pp_cuda_mem(f'After {layer_idx} conv2d {j}'))
        else:
    ...
    # [batch_size, k]
    if layer_idx != len(net.layers)-1:
        print(pp_cuda_mem(f'After backsub {layer_idx}'))
    else:
        print(pp_cuda_mem(f'After Final backsub'))
    return ret_l, ret_u
...
def get_diffs_C(targets, nb_classes=10):
    ...
    c1 = torch.eye(nb_classes).to(targets.device)[targets].unsqueeze(1)
    # [1, nb_classes, nb_classes]
    c2 = torch.eye(nb_classes).unsqueeze(0).to(device=targets.device)
...
def forward_deeppoly(net, inputs, targets, eps, data_range, bounds=None, crown_variant=None, is_iclr2021=False, C='diffs', soft_slope_gamma=None):
    ...
    print(pp_cuda_mem('Before deeppoly'))
    for i, layer in enumerate(net.layers):
    ...
    print(pp_cuda_mem('After deeppoly'))
...
def forward_hybridzono(net, x, bounds=None, C=None, soft_slope_gamma=None, zono_kappa=None, loosebox_round=None, loosebox_widen=None, dcs_per_one=None):
    ...
    for i, layer in enumerate(net.layers):
        if isinstance(layer, nn.ReLU):
            ...
            print(pp_cuda_mem('before zono-relu'))
